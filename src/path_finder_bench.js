...
function benchmarkMap(map, name, iterations) {
  ...
  benchmark('PathFinder.find '+name, iterations, function() {
...
benchmarkMap(WorldMapFactory.build5x5empty(), '5x5 empty', 1e6);
benchmarkMap(WorldMapFactory.build5x5WithCenterObstructions(), '5x5 center walls', 1e5);
benchmarkMap(WorldMapFactory.buildJpsPlusSample(), 'JPS+ sample map', 1e5);
benchmarkMap(WorldMapFactory.buildJsRtsSampleMap(), 'JS-RTS sample map', 1e4);
benchmarkMap(WorldMapFactory.build128x128rooms(), '128x128 rooms map', 100);
